# the purpose of this script is to compare the NOW tabloids with the NOW global news
library(tidyverse)
...
tabloids <- NOW_subsets$tabloids
major_global_news <- NOW_subsets$major_global_news
major_global_news_cleaned <- setdiff(major_global_news, tabloids)

vars <- c(binary_vars, cont_vars)
#vars <- c("words_n", "is_NP")
# for the NOW corpus
for (var in vars) {
  
  # create a directory if it doesn't exist yet
  dirname <- paste0("/mnt/home/nickl/Users/NicklPietro/PaperIIwb/plotting/final_figures/figure_3/figure_3_b/NOW_cleaned/", var)
  ...
  NOW_tabloids <- NOW[NOW$source_domain %in% tabloids, ]
  
  NOW_major_news <- NOW[NOW$source_domain %in% major_global_news_cleaned, ]
  
  reference_corpora_plot <- plot_reference_corpora(var)
  
  tabloid_plot <- plot_corpus_color(var, NOW_tabloids, "red")
  
  major_news_plot <- plot_corpus_color(var, NOW_major_news, "blue") 
  
  combined_plot <- reference_corpora_plot + 
    tabloid_plot$layers + 
    major_news_plot$layers + 
    theme_minimal() +
  ...
  # save plot 
  filepath <- paste0(dirname, "/tabloids_vs_major.jpg")
